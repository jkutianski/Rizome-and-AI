...
def canonical_prompt_inventory():
    if not CANONICAL_PROMPTS.exists():
        return set()
    return set(CANONICAL_PROMPTS.glob("*.prompt.md"))


def prompt_name(prompt_file, agent):
    if agent == "copilot":
        return prompt_file.name
    return prompt_file.name.removesuffix(".prompt.md") + ".md"


def prompt_content(prompt_file, agent):
...
def prompt_projection_is_valid(target, source, agent):
    if not target.is_file():
        return False
    content = target.read_text(encoding="utf-8")
    if source.name not in content or "prompts/" not in content:
        return False
    if agent == "copilot":
...
def check_generated_agent_prompts():
    print("\n== Generated agent prompt projections ==")
    prompts = canonical_prompt_inventory()
    if not prompts:
        print("ERROR    No canonical prompts found: prompts/")
        errors.append("prompts/")
        return
    for agent, target_root in AGENT_PROMPT_ROOTS.items():
        label = target_root.relative_to(ROOT)
        print(f"\n-- {agent.upper()}: {label} --")
        if not target_root.exists() or not target_root.is_dir():
            print(f"ERROR    Missing prompt root: {label}")
            errors.append(str(label))
            continue
        marker = target_root / GENERATED_MARKER
        if not marker.is_file() or marker.read_text(encoding="utf-8") != PROMPT_MARKER_CONTENT:
            print(f"ERROR    Missing or invalid prompt marker: {label}/{GENERATED_MARKER}")
            errors.append(str(marker.relative_to(ROOT)))
        expected = {prompt_name(prompt, agent) for prompt in prompts}
        actual = {
        ...
        for missing in sorted(expected - actual):
            print(f"ERROR    {agent}: missing prompt projection: {missing}")
            errors.append(f"{label}/{missing}")
            valid = False
        for stale in sorted(actual - expected):
            print(f"ERROR    {agent}: unexpected prompt projection: {stale}")
            errors.append(f"{label}/{stale}")
            valid = False
        for source in prompts:
            target = target_root / prompt_name(source, agent)
            if not target.is_file():
                continue
            if not prompt_projection_is_valid(target, source, agent):
                print(f"ERROR    {agent}: out of sync: {target.relative_to(ROOT)}")
                errors.append(str(target.relative_to(ROOT)))
                valid = False
        if valid:
            print(f"PASS     {agent}: {len(expected)} prompt projections synchronized")
